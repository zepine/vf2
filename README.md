...
```
(예) Set-executionPolicy -executionPolicy **undefined** -scope currentuser
```
...
- 모듈 다운로드/실행
  ...
  - 다시 말해, package.json 파일은 모듈 설치를 위한 필수 준비 작업. <strike>`node init`</strike>이 아님.
...
3. 프로젝트 폴더 이동 `cd <folder name>`
4. 서버 실행 `npm run serve`
5. 메인 페이지 ./src/App.vue
...
5. git에 remote repository 추가(연동)
...
2. Firebase설치하기 (terminal)
   ...
   1. [Firebase CLI 테스트](https://firebase.google.com/docs/cli?authuser=2#sign-in-test-cli)
      1. Firebase 로그인 
      `firebase login`
      2. Firebase 프로젝트 리스트 불러오기
      `firebase projects:list`
      3. CI 시스템과 함께 CLI 사용 [link](https://firebase.google.com/docs/cli?authuser=2#cli-ci-systems) (그냥 따라 해 봄)
        - Visit this URL on this device to log in:
        ```
        https://accounts.google.com/o/oauth2/auth?client_id=563584335869-fgrhgmd47bqnekij5i8b5pr03ho849etform&response_type=code&state=124571468&redirect_uri=http%3A%2F%2Flocalhost%3A90056.apps.googleusercontent.com&scope=email%20openid%20https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fcloudplatformprojects.readonly%20https%3A%2F%2Fwww.googleapis.com%2Fauth%2Ffirebase%20https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fcloud-platform&response_type=code&state=124571468&redirect_uri=http%3A%2F%2Flocalhost%3A9005
        ```
        - firebase_token : 
        ```
        1//0eHnT1j7i1mMWCgYIARAAGA4SNwF-L9IrseWNMeCyk0Lxg3pBONi40u_y4EmCCH93YBWSywSuSFOfP4sl3QTUMZTxoUv27W3E7Ds
        ```
        - Example :
        ```
   ...
   2. [Firebase 프로젝트 초기화](https://firebase.google.com/docs/cli?authuser=2#initialize_a_firebase_project)
      1. 프로젝트 디렉토리(폴더) 생성
  ...
  2. `firebaseConfig` (앱의 키 및 식별자가 포함된 구성 객체 - Firebase console의 '내 앱')을 외부 파일로 만들고,
  firebaseConfig.js
      ```
      export default {
      ...
      ```
  1. firebase.js에서 improt 함
  `import firebaseConfig from '../../firebaseConfig'`
  1. main.js에서 firebase, import 하기
